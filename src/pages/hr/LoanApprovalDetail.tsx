...
const LoanApprovalDetailPage = () => {
  ...
  useEffect(() => {
    setDetail(applicationFromState);
  }, [applicationFromState]);

  useEffect(() => {
    if (!detail) return;
    const match = loansFromState.find((l) => l.loan_application === detail.name);
    if (match) {
      setLinkedLoan({ ...match, rawStatus: match.status });
    }
  ...
  const handleApproveReject = async () => {
    ...
    try {
      ...
      setModalOpen(null);
      setRemarks("");
      navigate("/hr-dashboard/loan-approvals", { replace: true });
    } catch (err: any) {
  ...
  const handleDisburse = async () => {
    ...
    try {
      const result = await (await import("../../services/admin.service")).disburseLoan(createResult.loan, disbursementDate);
  ...
  return (
    <div className={`min-h-screen ${isDark ? "bg-gray-900 text-white" : "bg-gray-50 text-gray-900"}`}>
      ...
      <div className="px-4 py-4 space-y-4 pb-8">
        <button onClick={() => navigate("/hr-dashboard/loan-approvals")} className={`flex items-center gap-2 text-sm ${isDark ? "text-gray-400 hover:text-white" : "text-gray-600 hover:text-gray-900"}`}>
          ...
          <>
            <div className={`rounded-2xl p-5 ${cardStyle}`}>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><p className={`text-[10px] uppercase tracking-wider ${isDark ? "text-gray-500" : "text-gray-400"}`}>Company</p><p className="text-sm font-semibold">{(detail as any).company || "-"}</p></div>
                ...
                <div><p className={`text-[10px] uppercase tracking-wider ${isDark ? "text-gray-500" : "text-gray-400"}`}>Loan Amount</p><p className="text-sm font-semibold">{(detail.loan_amount ?? 0).toLocaleString()} SAR</p></div>
                ...
                <div><p className={`text-[10px] uppercase tracking-wider ${isDark ? "text-gray-500" : "text-gray-400"}`}>Repayment Periods</p><p className="text-sm font-semibold">{detail.repayment_periods ? `${detail.repayment_periods} months` : "-"}</p></div>
                <div><p className={`text-[10px] uppercase tracking-wider ${isDark ? "text-gray-500" : "text-gray-400"}`}>Monthly Repayment</p><p className="text-sm font-semibold">{(detail.repayment_amount ?? 0).toLocaleString()} SAR</p></div>
                <div><p className={`text-[10px] uppercase tracking-wider ${isDark ? "text-gray-500" : "text-gray-400"}`}>Total Payable</p><p className="text-sm font-semibold">{(detail as any).total_payable_amount?.toLocaleString?.() ?? "-"} SAR</p></div>
                <div><p className={`text-[10px] uppercase tracking-wider ${isDark ? "text-gray-500" : "text-gray-400"}`}>Total Interest</p><p className="text-sm font-semibold">{(detail as any).total_payable_interest?.toLocaleString?.() ?? "-"} SAR</p></div>
                <div><p className={`text-[10px] uppercase tracking-wider ${isDark ? "text-gray-500" : "text-gray-400"}`}>Secured Loan</p><p className="text-sm font-semibold">{(detail as any).is_secured_loan ? (detail as any).is_secured_loan === 1 ? "Yes" : "No" : "-"}</p></div>
