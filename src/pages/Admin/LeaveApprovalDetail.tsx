...
const LeaveApprovalDetailPage = () => {
  ...
  const [activeMenu, setActiveMenu] = useState("leave-approvals");
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      ...
      try {
        setLoading(true);
        const data = await getLeaveApprovalDetail(decodeURIComponent(name));
        setDetail(data);
      } catch (err) {
        console.error("[LeaveApprovalDetail] Failed to fetch:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      if (modalOpen === "approve") {
        await approveLeaveRequest(decodeURIComponent(name), remarks || undefined);
      } else {
        await rejectLeaveRequest(decodeURIComponent(name), remarks || undefined);
      }
      setModalOpen(null);
      setRemarks("");
      navigate("/admin/leave-approvals", { replace: true });
    } catch (err: any) {
      console.error("[LeaveApprovalDetail] Action failed:", err);
  ...
  return (
    <div className={`min-h-screen ${isDark ? "bg-gray-900 text-white" : "bg-gray-50 text-gray-900"}`}>
      <AdminHeader onMenuToggle={() => setShowSidebar(true)} title="Leave Approval Details" />
      <AdminSidebar isOpen={showSidebar} onClose={() => setShowSidebar(false)} activeMenu={activeMenu} onMenuChange={setActiveMenu} />

      <div className="px-4 py-4 space-y-4 pb-8">
        <button onClick={() => navigate("/admin/leave-approvals")} className={`flex items-center gap-2 text-sm ${isDark ? "text-gray-400 hover:text-white" : "text-gray-600 hover:text-gray-900"}`}>
          ...
          <>
            <div className={`rounded-2xl p-5 ${cardStyle}`}>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><p className={`text-[10px] uppercase tracking-wider ${isDark ? "text-gray-500" : "text-gray-400"}`}>Leave Type</p><p className="text-sm font-semibold">{detail.leave_type}</p></div>
                ...
                <div><p className={`text-[10px] uppercase tracking-wider ${isDark ? "text-gray-500" : "text-gray-400"}`}>Total Days</p><p className="text-sm font-semibold">{detail.total_leave_days} days</p></div>
                <div><p className={`text-[10px] uppercase tracking-wider ${isDark ? "text-gray-500" : "text-gray-400"}`}>Status</p><p className="text-sm font-semibold">{detail.status}</p></div>
              </div>
              {(detail.reason || detail.description) && (
                <div className="mt-4">
                  <p className={`text-[10px] uppercase tracking-wider ${isDark ? "text-gray-500" : "text-gray-400"}`}>Reason</p>
                  <p className="text-sm mt-1 whitespace-pre-wrap">{detail.reason || detail.description}</p>
                </div>
              )}
            </div>
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className={`fixed inset-0 z-50 flex items-center justify-center p-4`}>
              <div className={`w-full max-w-sm rounded-2xl p-6 shadow-2xl ${isDark ? "bg-gray-800" : "bg-white"}`}>
                <h3 className="text-lg font-bold mb-2">{modalOpen === "approve" ? "Approve" : "Reject"} Leave Request</h3>
                <p className={`text-sm mb-4 ${isDark ? "text-gray-400" : "text-gray-500"}`}>Are you sure you want to {modalOpen} this leave request?</p>
...
export default LeaveApprovalDetailPage;
